...
| `SettingsPage` | `apps/web/src/pages/SettingsPage.tsx` | Tabbed layout: **Options** (Color Scheme, Default Sort (Recipes), Default Sort (Exercises), Tags), **Goals** (nutrition daily macros + workout weekly goals + per-routine session goals + body measurement goals — all in one place), **User** (change username/password), **Delete Data** (danger zone), **Export** (date range pickers + download all data as xlsx). Left-aligned layout matching History/Links pages. |
...
| `RoutineDetailPage` | `apps/web/src/pages/RoutineDetailPage.tsx` | Routine editor — editable name, routine type picker (dropdown in header subtitle), last session metric in header (type-aware: lbs/steps/mi/min), type-aware line chart, exercise blocks with template sets (blur-save, includes steps column), last-performed sets as reference, Start Routine button. |
...
- **Migration 018**: `018_routine_type.sql` adds `routine_type ENUM('strength','bodyweight','cardio_distance','cardio_duration','steps') NOT NULL DEFAULT 'strength'` to `workout_routines` via post-migration hook. Applied manually on EC2: `ALTER TABLE workout_routines ADD COLUMN routine_type ENUM('strength','bodyweight','cardio_distance','cardio_duration','steps') NOT NULL DEFAULT 'strength';`
- **Migration 019**: `019_steps_field.sql` adds `steps INT NULL` to both `exercise_sets` and `routine_exercise_sets` via post-migration hooks. Applied manually on EC2: `ALTER TABLE exercise_sets ADD COLUMN steps INT NULL; ALTER TABLE routine_exercise_sets ADD COLUMN steps INT NULL;`
- **Migration 020**: `020_routine_goals.sql` creates `routine_goals` table (`id`, `routine_id` FK, `user_id` FK, `target_per_week` INT, `effective_from` DATE, UNIQUE on `(routine_id, user_id)`). `GET /api/routines/goals` returns all goals for the current user. `PUT /api/routines/:id/goal` upserts a goal. `DELETE /api/routines/:id/goal` removes it.
...
- **Exercise trackedFields**: `tracked_fields` VARCHAR(100) stores a comma-separated list of which inputs to show per set: `reps`, `weight`, `duration`, `distance`, `steps`. Defaults per type: weight → `reps,weight`; bodyweight → `reps`; cardio → `duration,distance`; duration → `duration`. User can override any combination (e.g. stairs = `duration,steps`). Both the workout detail fetch and the add-exercise response include this field from the exercises JOIN. Duration is stored as `duration_seconds` (integer) and displayed/entered as MM:SS. Steps stored as integer in `exercise_sets.steps` and `routine_exercise_sets.steps`.
...
- **Routine type**: `workout_routines.routine_type` ENUM drives metric display throughout the app — `strength`/`bodyweight` → volume in lbs; `steps` → step count + duration; `cardio_distance` → miles; `cardio_duration` → minutes. The `WorkoutSummary` type includes `routineType: string | null` so dashboards can switch display without fetching the routine. All `/stair/i` regex hacks have been replaced with `routineType` checks.
- **Per-routine goals**: `routine_goals` stores a `target_per_week` sessions goal per routine per user. Used by the dashboard This Week bar chart to normalize bar heights: strength bars = `volumeLbs / volumeGoal`; non-strength bars = `sessions / targetPerWeek` (fallback 3 if unset). Goals set in Settings → Goals → "Per-Routine Goals". `routinesApi.getAllGoals()` returns all goals; `routinesApi.setGoal(id, n)` upserts; `routinesApi.deleteGoal(id)` removes.
- **Workout update date safety**: `PUT /api/workouts/:id` only includes `workout_date` in the SET clause when `workoutDate` is explicitly passed in the request body. Previously it defaulted to today, causing date resets when updating other fields from history view.
- **Workout completed field**: `WorkoutDetail` type now includes `completed: boolean`. Web and mobile both guard timer start with `!w.completed` so viewing a completed workout from history never restarts the elapsed timer.
