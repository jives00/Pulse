...
### Frontend – Web
- **Setting a goal stopped asking you for a number the app already had** — the Add Goal wizard's "Starting value" opened blank even for metrics Pulse tracks, so a weight goal meant going to find your own weight first. Step 3 now fetches the metric's present reading and fills it in, with a "Your current value" hint; typing your own number wins, and a late response can't stomp it `e19bdd2`

### Frontend – Mobile
- **Setting a goal stopped asking you for a number the app already had** — same prefill in the mobile Add Goal sheet `e19bdd2`
