...
- **The routine cover photo stopped pushing the exercise list down the page** — the right column of a routine page led with a square cover image, which on a wide screen is tall enough that the Exercises heading started below the fold. It is now 16:9 and capped at 224px; `object-cover` means the photo crops rather than shrinks `47d18a2`
