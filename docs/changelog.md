...
### Frontend – Web
- **Blurb cards square off** — the two blurb cards are the only dashboard cards that don't go through `Panel`; they render their own container and hardcoded `borderRadius: 10` while `Panel` uses `0`, leaving them the only rounded cards on the page. Weekly changed alongside Today's since it shares the style `3c8f516`
