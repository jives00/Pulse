...
### Backend
- **`GET /api/goals-v2/current-value`** — what a metric reads right now for a goal that does not exist yet, so the Add Goal form can prefill the starting value. Reuses the existing `CURRENT_VALUE_SQL` against a one-row derived table shaped like the goal about to be created, which is what keeps a prefill from drifting away from the number the goal card will show once it is saved. Keys with no authoritative source, and metrics with no data, return `null` `12528e5`
- **An expired token stopped meaning "log in again"** — the JWT lives 7 days and there is no refresh endpoint, so it expires on a schedule; a 401 then logged the user out and showed the login screen. The passwordless `/auth/session` route already existed, but the mobile launch path only reached it when there was *no* stored token at all, so an expired one skipped it entirely — a guaranteed login screen roughly weekly, plus any 401 mid-session. The shared api-client now takes an optional `recoverSession` hook: on a 401 it mints a fresh token and retries the request once, and `onUnauthorized` only fires after that has also failed. `/auth/session`, `/auth/login` and `/auth/register` are exempt so recovery can't recurse and a bad password stays a bad password. Web leaves the hook unset and behaves exactly as before `190dcef`
- **The login limiter stopped locking the only real user out** — a flat 10 per 15 minutes covered `/login` *and* `/session` for every caller, trusted or not. Throttling the trusted network protected nothing, since `/session` hands that same account a full token from those exact IPs with no password, while the mobile app re-probing a flaky connection could burn the budget — and the resulting 429 rendered on screen as a bad password. It now skips trusted callers and allows 20 per 5 minutes for everyone else. Safe to key on because `isTrustedRequest` reads `socket.remoteAddress`, never `X-Forwarded-For`, so tunnelled requests are still limited `190dcef`
- **Routines can be archived instead of deleted** — a routine you have stopped doing had two options: leave it cluttering the grid, or delete it and take its logged sessions with it. `workout_routines.archived_at` (migration 044) adds a third: `GET /api/routines` takes `archived=exclude|include|only` and defaults to `exclude`, so every existing caller — the planning picker, the goal source picker, mobile quick-start — drops archived routines with no change, while `PUT /:id { archived }` toggles the flag. Nothing touches `workout_logs`, so history, stats, dashboards and the Excel export read exactly as before. `/api/schedules/upcoming` also skips archived routines, including ones named inside a `custom_cycle`, so a hidden routine cannot reappear on the calendar through a schedule that outlived it `2232f87`
- **`POST /api/steps/bulk`** — backfill entrypoint for Health Connect, taking a window of `{ date, steps, overwrite? }` days. Past days merge with `GREATEST`, so a sync that catches a day mid-morning can no longer replace a finished total with its partial count; only the day the client is currently living in may move a stored value downward, since that one legitimately grows all day. Assignment order inside the `ON DUPLICATE KEY UPDATE` is load-bearing — MySQL evaluates it left to right, so `source` and `logged_at` are set before `steps` while the old value is still readable. Single-day `POST /api/steps` still overwrites unconditionally, which is what manual entry wants `10954b0`

### Frontend – Web
- **Setting a goal stopped asking you for a number the app already had** — the Add Goal wizard's "Starting value" opened blank even for metrics Pulse tracks, so a weight goal meant going to find your own weight first. Step 3 now fetches the metric's present reading and fills it in, with a "Your current value" hint; typing your own number wins, and a late response can't stomp it `12528e5`
- **Archived routines, and a routine order that means something again** — the routines grid gets a "Show archived (n)" toggle, archived cards dim with an `Archived` badge, and the routine detail page gets an Archive/Unarchive button beside Delete. Ordering moved into a shared `compareRoutines`: scheduled routines first by soonest date, then unscheduled ones least-recently-used first, never-used after those, archived below everything, name breaking ties. The last-used tier had been dropped in `4031dd8` when next-occurrence sorting landed, which left the unscheduled tail falling back to the server's alphabetical order `2232f87`
- **Creating an exercise now continues straight into filling it in** — the New Exercise form takes a name, category and type, then dropped you back on the grid; every detail that makes an exercise useful (steps, muscles, cover image, media) lived behind the Edit button on the detail page, which meant finding the new card and opening it by hand. Saving now navigates to the new exercise's page with the Edit modal already open, via an `edit=1` param the page strips once consumed so a refresh or back-nav doesn't reopen it. Mobile already used one modal for both `6bab367`
- **The routine cover photo stopped pushing the exercise list down the page** — the right column of a routine page led with a square cover image, which on a wide screen is tall enough that the Exercises heading started below the fold. It is now 16:9 and capped at 224px; `object-cover` means the photo crops rather than shrinks `241ba63`
- **Weekly goal progress learned which direction each goal points** — the card scored every metric as "hit the number", so a calorie deficit — the entire point of the goal — rendered as a red **Behind** badge, while running ahead on protein earned nothing. Targets now carry a direction: calories are a *ceiling* (**Under** and **On budget** read green, **Trending over** warns, and going past the week's total turns the bar red), while protein, water, workouts and volume are *floors* and keep the Behind/Close/On pace/Done ladder. **Water** joins the card, in 8 oz glasses to match the daily nutrition card, and calories/protein/water now honour the explicit `weekly_*` goal columns where they are set instead of always multiplying the daily goal by 7. The metrics also share one equal-width row rather than wrapping after three. The pace logic itself moved to `packages/api-client/src/weeklyPace.ts` so the two platforms cannot disagree about what counts as on pace `c4a2831`

### Frontend – Mobile
- **Setting a goal stopped asking you for a number the app already had** — same prefill in the mobile Add Goal sheet `12528e5`
- **Archived routines on mobile** — same toggle (as a list footer), dimmed cards and Archive/Unarchive on the routine screen; long-pressing a routine card now offers Archive alongside Delete rather than jumping straight to a delete prompt. Both copies of the tab's inline sort comparator were replaced by the shared helper `2232f87`
- **Launch stopped giving up on the first failed probe** — the app is usually opened the moment the phone wakes, before wifi or Tailscale have settled, and a single unreachable `/health` probe resolved to "no base" and sent the user to the login screen. Bootstrap now retries three times with a short backoff, re-probing between attempts, and the probe timeout went from 2.5s to 4s `190dcef`
- **The login screen stopped blaming the password for everything** — every failure read "Invalid username or password", including a dead NAS, a disconnected Tailscale and a 429 from the limiter, so a network blip got retried until it became a real lockout that still read as a typo. Failures are now classified per status; only a 401 implicates the password and only that renders in the error colour. Added a **Retry connection** link that re-probes the API bases and signs straight in via `/session` — what a force-close used to do `190dcef`
- **Steps taken abroad stopped landing on the wrong day** — steps were read from device-local midnight but written under a hardcoded `America/Chicago` date. Those agree at home and diverge on a trip, so syncing during a European morning filed that day's partial count onto the *previous* US date and overwrote a complete total with a few hundred steps — an active day showing up as a quiet one. Days are now bucketed by Health Connect's own period-based daily aggregation and labelled with the phone's current local date, so a step counts against the day it was walked in. The sync also reads a trailing 30-day window instead of today alone: a single-day read left every day the app was never opened permanently blank, since nothing revisited them, and 30 days matches Health Connect's retention — so gaps and truncated days now repair themselves on the next foreground rather than staying wrong forever `10954b0`
- **Weekly goal progress learned which direction each goal points** — same directional pace ladder, water section in glasses, and weekly-column goal fallbacks, off the shared `weeklyPace` helper. Mobile keeps its vertical stack rather than the web card's equal-width row `c4a2831`

## August 24, 2026

### Backend
- **Today's workout stopped disappearing every evening** — the three `/upcoming` routes (workout schedules, meal schedules, nutrition schedules) built their "today" from `new Date()` on the server. The container sets no `TZ`, so it runs in UTC and rolls over to tomorrow at 7pm Central: from then until midnight the window started on the wrong day and today was never in the response at all. The dashboard and planning calendar both filter on the *local* date, so a workout that was still scheduled simply showed nothing — every day, for the last five hours of it. Clients now send their own local date as `from` and the server clock is only a fallback, via a shared `resolveFromDate` that round-trips the parse (`Date.parse` accepts `2026-02-31` and quietly rolls it into March). The param is appended inside the api-client `getUpcoming` helpers, so web and mobile are both covered without touching a single call site `13716b5`

## August 23, 2026

### Backend
- **Uploaded media recorded what it actually was** — exercise media is stored under a key like `exercises/12/media/1724445000` with no extension, and there is no content-type column, so nothing anywhere recorded whether an object was a still or a video clip and the renderers had no way to tell. All six exercise upload routes now append the extension for the content type via a shared `keyWithExtension`, and `media-from-url` keeps the type it read off the response rather than defaulting to JPEG. Existing keys are extensionless and all images, so they keep rendering as before `7ea3e1a`
- **Shared media-type helpers** — `mediaExtensionFor`/`keyWithExtension`/`isVideoMedia` live in `packages/api-client/src/utils/media.ts` so the server that writes the key and the two clients that read it can't disagree about what counts as a video. `isVideoMedia` strips the query string first, since a presigned S3 URL carries the AWS signature params after the key `7ea3e1a`

### Frontend – Web
- **Video exercise demos played instead of showing nothing** — `MediaEmbed` rendered a YouTube iframe or fell through to `<img src>`, and an `<img>` pointed at an mp4 draws nothing at all: the upload had succeeded, the demo was simply invisible. There were two copies of the component, so both the exercise detail page and the active-workout screen were affected; each now renders a looping muted `<video>` for video media, with controls on the detail page and without during a workout, where it should just loop like the demo it replaces `7ea3e1a`
- **Failed uploads said so** — the S3 PUT in `uploadFromFile` was never checked, and `fetch` does not throw on a 4xx, so a rejected upload was reported as a success and the exercise was saved pointing at an object that was never written. The PUT status is now checked and S3's `<Message>` pulled out of its XML error body, and the URL-upload path surfaces the server's own error text rather than a blanket "check the URL" `7ea3e1a`

### Frontend – Mobile
- **Video exercise demos played instead of showing nothing** — the demo was rendered with React Native `<Image>`, which cannot play video, and no video package was installed at all. Added `expo-video` and a `DemoMedia` component that picks a player or an image by media type; the hook lives in its own component so it isn't called conditionally `7ea3e1a`
- **Videos could be picked at all** — the image picker was hard-coded to `MediaTypeOptions.Images`, so a demo clip couldn't even be selected for the media field, which is the one field meant to hold one. Cover and muscle images stay images-only `7ea3e1a`
- **Failed uploads said so** — same unchecked S3 PUT as on web, with the same silent success `7ea3e1a`
- **The food log stopped jumping to the bottom when water was logged** — tapping "+ glass" refetched the entire daily log, re-rendering every meal section and food row. On Android a re-render of a `ScrollView`'s children can hand focus to a different child, and the `ScrollView` scrolls to whatever gains focus — usually the last row, which is why it only happened some of the time. A water add changes exactly one number, so `waterTotalOz` is now patched in local state and the meal list never re-renders; a date guard drops a slow response that lands after the day was changed `4740254`
- **Food log opened on yesterday** — the nutrition screen picked its date once, in `useState(localDateStr(new Date()))`, and Android keeps the app process and that screen alive for days, so opening the app the morning after an evening of logging restored yesterday's selection and every entry had to be moved by hand. An `AppState` listener now compares the calendar day against the day the app was last active and snaps back to today when it rolled over; backgrounding the app briefly while paging through an earlier day leaves the selection alone, since only a real day change resets it. The move/copy destination date needed nothing — it is re-seeded each time that modal opens `83b6e41`
- **Evening dates were logged as tomorrow** — `new Date().toISOString().slice(0, 10)` is the *UTC* day, which rolls over at 7pm Central, so anything defaulted from it after dinner was dated a day ahead. The new body measurement in History defaulted that way, its range filter shifted both ends of the window, and every date picker whose stored value was still empty fell back to `new Date()`, carrying the current time-of-day into the picked result — goal deadline, routine start/end, planning target date, measurement date. All of them now go through `localDateStr`, and the pickers seed from a noon-anchored local date, which also makes the `selected.toISOString()` in their `onChange` handlers safe. The 30- and 56-day cutoffs on the routine detail chart got the same treatment; those only shifted a boundary by a day `83b6e41`

## August 15, 2026

### Backend
- **Described meals were estimated per 100g and logged as totals** — Describe a Meal called `estimateMacros`, whose prompt and tool schema both ask for values per 100g, then logged what came back verbatim as the meal's totals: "Noodles & Co. Butter Noodles with Meatballs" returned 320 kcal, a fair per-100g figure for a ~1100 kcal dish, with the other macros off by the same factor. New `estimateMeal` and `POST /api/foods/estimate-meal` ask for totals for the whole portion described and tell the model to use the named chain's actual menu item rather than anchoring low. `estimateMacros` keeps its per-100g contract for the food database, which is what the custom-food form and the Foods page actually want `c1de995`
- **Goal names lost their redundant " Goal" suffix** — both add-goal forms defaulted the name to `catalogLabel + ' Goal'`, so goals created without a typed-in name read "Weight Goal" while any hand-named goal didn't, leaving the list inconsistent. The word carries nothing: these only appear under a Goals heading or on a goal card. Both forms now default to the bare catalog label, and migration `043_trim_goal_name_suffix.sql` strips a trailing " Goal" from existing rows — trailing only, so "Goal Weight" is left alone, and never down to an empty name `b41f045`
- **Goal progress since a date** — new `GET /api/goals-v2/since?date=YYYY-MM-DD` returns, for every active goal, the reading on that date, the reading today, and the difference. The reading for a date is not simply "the row logged that day": most days have no weigh-in, so point metrics take the nearest reading on or before the date (falling back to the first one after, so a goal started mid-window still reports something), while the daily-average metrics take the same trailing 30-day mean the goal card uses for its current value — otherwise a single noisy day would be compared against a 30-day average and the gap called progress `b41f045`
- **One source of truth for a goal's history** — the per-catalog-key SQL behind `GET /:id/progress` was an eight-branch if-ladder inside the route handler, so the new endpoint would have had to copy it. It now lives in `seriesQueryFor`/`loadGoalSeries`, which both routes read through, and each query carries a `{{DATE}}` placeholder so a lower bound can be spliced in without duplicating the query. Behavior is unchanged: auto-tracked goals still return synthetic negative ids, manual goals still return their real `goal_progress` rows so the UI can delete them `b41f045`
- **Baseline maths are shared, not server-only** — `pointAt`/`averageAt`/`readingAt` live in `packages/api-client/src/goalSince.ts`. They only run on the server, but keeping them out of the route file means they can be tested without a database `b41f045`
- **Per-widget settings in preferences** — `dashboardLayout` gains a `widgetSettings` key, merged independently of the `web`/`mobile` halves. Deliberately not per-platform: "progress since Jan 1" has to mean the same thing on the phone as on the laptop, and setting the date twice would be a bug. Holds both the baseline date and the goal selection, each written without clobbering the other `b41f045`
- **Goal measurements rounded away their second decimal** — `GET /goals-v2` and the progress history rounded every value to one decimal on the way out, and the WeightGurus sync rounded the same way on the way in, so a 182.45 lb reading was stored and reported as 182.5. All four now round to hundredths, which the `DECIMAL(8,2)`/`DECIMAL(10,3)` columns already hold, and the shared `goalSince` baseline average matches `d2e0226`
- **Steps history returned unusable dates** — `GET /api/steps/history` serialized `log_date` with `String(r.date).slice(0, 10)`, but `log_date` is a DATE column so mysql2 hands back a `Date`: that produced `"Thu Aug 13"` and was off by a day. No client could match it against a `YYYY-MM-DD` key, which silently broke the step-kcal line in Calories consumed-vs-burned and the step total in the weekly blurb on both platforms. Now uses the same `instanceof Date` guard as the water and history routes `f244957`
- **New widgets land where they belong instead of at the bottom** — `resolveLayout` appended any widget a stored layout had never seen, so a new Trends card landed below the blurbs and `groupLayout` — which only merges *consecutive* same-group widgets — opened a second "Trends" section header for anyone with a saved layout. A newcomer now splices in after its nearest present catalog predecessor, so it follows the widget it ships behind even in a reordered layout, and several adjacent new widgets chain correctly `f244957`
- **Shared steps aggregation** — new `buildStepsStats` in `packages/api-client/src/utils/steps.ts` backs both step cards so they can't drift on what "7-day average" means. Unlogged days come back `null` rather than `0`, and averages run over logged days only — otherwise a week with two missing syncs reads as a bad week instead of an incomplete one. Totals still span the whole window, and a 0-step row counts as unlogged `f244957`
- **`section` accepted on stored layout entries** — the dashboard layout schema takes an optional per-widget `section`, so a widget the user moved out of its catalog band survives a reload. Validated as a plain optional string here and narrowed to a real band by `resolveLayout`, which drops an override naming a section that no longer exists rather than rendering a header for it `8e13353`

### Frontend – Web
- **Describe a Meal estimates the portion you typed** — the AI estimate button now calls the meal endpoint, so the numbers that land in the four macro boxes are for the whole meal rather than 100g of it `c1de995`
- **Goal progress since a date** — new dashboard card in the Goal progress band: pick a date and each goal shows what it read then, what it reads now, and the change, colored by whether that change moved toward the target rather than by its sign — losing five pounds of muscle mass against a bulk target is not green. A goal picker chooses which goals appear; leaving it untouched follows every active goal, so a goal added next month shows up on its own, while an explicit pick stays exactly as picked. The two reading dates label the columns once rather than repeating on every row. Goals with no reading on either end are dropped rather than shown as two em-dashes. Date and selection both persist to preferences, so they're the same on mobile. The Goal progress band no longer disappears wholesale when nothing is pinned — only the `goalProgress` card does, since this one has plenty to show for an unpinned goal `b41f045`
- **Goal values show two decimals** — `fmtGoalValue` and every goal/measurement `toLocaleString` call dropped everything past the first decimal, so entering a waist of 32.25 in read back as 32.3. The shared formatter is now `fmt2` — up to two decimals with trailing zeros trimmed, so 2.5 stays 2.5 rather than becoming 2.50 — and the measurement input in History steps by 0.01 instead of 0.1 `d2e0226`
- **Steps · by day dashboard card** — 30-day bar chart with a hover tooltip (steps, estimated kcal, ± vs goal), a dashed goal line and a dashed 30-day-average line, goal-hitting bars in green and today at full opacity. Stat row covers today vs goal, 7-day average with a delta against the prior 7 days, 30-day average, best day, this week's total, and goal-hit days with the current streak. The target comes from an `activity_steps_daily_avg` goal; every goal-derived stat hides when there isn't one `f244957`
- **One type scale across the dashboard** — sizes had been picked per widget, so the same role rendered at anywhere from 8px to 15px depending on which component drew it: goal cards ran 2-4px behind the widgets sitting beside them in the same grid, and the streak card was drawing 8px text. `utils/typeScale.ts` now defines the eight roles (label 11, small 13, body 15, cardTitle 15, title 20, sub 22, kpi 26, hero 42) and the page, the goal cards, and the editor chrome all read from it. Everything is a step or two larger than before as part of the pass `8e13353`
- **Dashboard text is actually readable now** — the dimmest token (`MUTED2`, used for chart axes, table headers, session dates and meta) sat at 2.19:1 against the card fill, roughly watermark contrast, and muted body copy at 3.88:1 was under the threshold too. Raised the alpha to 0.8 and lightened the trakt palette's muted and border, giving 3.99:1, 5.33:1 body copy, 4.23:1 on the warning color and visible panel edges. `utils/dashboardTheme.ts` replaces six copy-pasted palettes — the duplication is why the old value could be fixed in one file and left broken in five `8e13353`
- **`--color-text` theme variable** — the dashboard hardcoded pure white and Tailwind slate for primary text regardless of the active palette, three different "primary text" colors on one page. The generator now emits each palette's own `text` value and the page uses it `8e13353`
- **Move a widget to another section** — the widget editor can send a card to any band rather than only reordering it within the one its catalog entry names. An untouched widget still follows its catalog group, so a future change to where a card ships lands for anyone who never moved it `8e13353`

### Frontend – Mobile
- **Describe a Meal estimates the portion you typed** — the quick-add estimate on the Nutrition tab moved to the same meal endpoint. The old per-100g `estimateMacros` wrapper had no other caller in mobile and is gone `c1de995`
- **Goal progress since a date** — the same card on the Goals tab, with a native date picker for the baseline, the same goal picker, and the same `buildGoalSinceRows` join and wording, so the two platforms can't disagree about whether a change counts as progress. Pull-to-refresh reloads the readings alongside everything else `b41f045`
- **Goal values show two decimals** — the Goals tab, the goal detail history, the progress list and the weekly blurb all match the web precision, since they read the same shared formatter `d2e0226`
- **Steps · by day dashboard card** — the same stats and the same `buildStepsStats` aggregation, drawn with the View-based bar technique the volume card uses plus an absolute-positioned goal line. `liveSteps` from Health Connect overrides today's stored row so the card isn't an hour stale, and the history fetch went from 14 to 30 days to fill the window `f244957`
