...
### Frontend – Web
- **Describe a Meal estimates the portion you typed** — the AI estimate button now calls the meal endpoint, so the numbers that land in the four macro boxes are for the whole meal rather than 100g of it `60ebe87`
...
### Frontend – Mobile
- **Describe a Meal estimates the portion you typed** — the quick-add estimate on the Nutrition tab moved to the same meal endpoint. The old per-100g `estimateMacros` wrapper had no other caller in mobile and is gone `60ebe87`
