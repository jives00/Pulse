...
- **Feature gating**: `featuresStore` (persisted as `dram-features`, hydrated from `GET /api/preferences` on `Layout` mount) holds the resolved module map. The persisted copy renders on first paint so there is no flash of soon-to-be-hidden nav. Gate UI with `useFeature(key)` / `<FeatureGate feature=…>` and routes with `<FeatureRoute feature=…>` (bounces to `/dashboard`) from `components/FeatureGate.tsx`. **When adding a page or dashboard widget, decide which module owns it** — nav entries carry `requires` in `Layout.tsx`'s `TOP_SECTIONS`, and widgets declare `requires` in the shared `dashboardCatalog.ts`. Features control *availability*; the dashboard layout controls *presentation*, and a disabled module always wins.
- **Dashboard layout**: `DashboardPage` renders from `resolveLayout(dashboardLayout, 'web', features)` through a `WIDGET_RENDERERS` registry into a single 12-column grid; section headers are derived from each widget's catalog `group`, so a group with nothing visible emits no kicker. Customize mode (topbar toggle) edits order/width/visibility via the pure helpers in `components/dashboard/layoutReducer.ts`, saved debounced. Per-goal card presentation lives in `components/dashboard/goalOrderReducer.ts` + `GoalOptionsPopover`, persisted to `goals.card_config`. **Fetches are gated on features, not on layout** — a widget merely hidden keeps its data loaded so re-showing it is instant.
...
- **API client**: `src/api/client.ts` — wraps the shared `@pulse/api-client` (axios). Base URL is resolved at runtime by `src/api/apiBase.ts`: it probes `/health` across the candidates in `src/api/config.ts` (`API_BASES` = baked `EXPO_PUBLIC_API_BASE` Tailscale IP + LAN IP `192.168.0.105:3000`) and uses whichever responds, so the app works on the LAN when Tailscale is down. The shared client's request interceptor awaits this resolver per request (fast-fails when nothing is reachable) — see `packages/api-client/src/client.ts` (`baseResolver`/`resolveBaseOnError`, wired in `app/_layout.tsx`).
- **Auth store**: `src/store/auth.ts` — Zustand + expo-secure-store, key `pulse-auth` (long-lived JWT). **Launch gate** (`app/index.tsx`): validates a stored token via `/auth/verify` or tries passwordless network auto-login (`/api/auth/session`) on a trusted network; shows the login screen only when the server is unreachable (without clearing the token, so a reconnect goes straight back in).
...
| `DashboardGoalCard` | `apps/web/src/components/goals/dashboard/` | Dispatcher picking one of four body variants (`trend`, `daily`, `streak`, `progress`) from `resolveGoalCard(goal.catalogKey, goal.cardConfig)`. `GoalCardShell` owns **all** chrome — title, target/deadline meta, status chip, stat row, legend — so no renderer formats its own header. Shared helpers (`fmtGoalValue`, `fmtDeadline`, `goalDirection`, `goalStatusFor`, `emptyMessageFor`) live in `goalCardHelpers.ts`; colors/sizes in `goalCardTheme.ts`. A goal with an unmapped catalog key falls back to `progress` rather than rendering nothing. Card order is `sortOrder` then `id`. |
| `SettingsPage` | `apps/web/src/pages/SettingsPage.tsx` | Tabbed layout: **Features** (feature-module toggles — sub-modules indent under their parent and grey out when it is off), **Options** (Color Scheme, Default Sort (Recipes), Default Sort (Exercises), Tags), **Goals** (nutrition daily macros + workout weekly goals + per-routine session goals + body measurement goals — all in one place), **Planning** (workout schedules, meal schedules, nutrition schedules, day-type presets, goal checkpoints — all via `PlanningCalendarCard`), **User** (change username/password), **Delete Data** (danger zone), **Export** (date range pickers + download all data as xlsx). Left-aligned layout matching History/Links pages. |
