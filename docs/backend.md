...
- **Session recovery on 401**: the JWT lives 7 days and there is no refresh endpoint, so it eventually expires. Mobile passes a `recoverSession` hook to `configureClient` (`packages/api-client/src/client.ts`); on a 401 the response interceptor calls it, and on success retries the original request with the new token. `onUnauthorized` (logout + login screen) only fires once recovery has also failed — i.e. genuinely off-network. `/auth/session`, `/auth/login` and `/auth/register` are exempt so recovery can't recurse and a bad password stays a bad password. Web doesn't set the hook and keeps the old behaviour.
- **Login limiter**: `loginLimiter` guards `/auth/login` and `/auth/session` at 20 per 5 minutes but `skip`s trusted requests. Throttling the trusted network protected nothing (`/session` gives that same account a token there with no password) while being the only thing able to lock the single real user out. Safe because `isTrustedRequest` reads `socket.remoteAddress`, never `X-Forwarded-For`.
...
/api/goals-v2/*        Unified goals (body/nutrition/exercise/activity) — GET /, GET /:id, POST /, PATCH /:id, DELETE /:id, POST /:id/close, GET /nudges, GET /since?date=YYYY-MM-DD, GET /current-value?catalogKey=&sourceId=; milestones: GET /milestones, GET|POST /:id/milestones, PATCH|DELETE /:id/milestones/:mid; progress: GET|POST /:id/progress, DELETE /:id/progress/:pid. Replaced /api/user-goals and /api/goal-checkpoints
