import type { WorkoutSummary, WorkoutExerciseSummary } from '../endpoints/workouts';
...
export function buildWorkoutLine(w: WorkoutSummary): string {
  const isRoutine = w.routineName != null;
  const name = isRoutine
    ? `${w.routineName} routine`
    : (w.name ?? (w.exercises.length === 1 ? w.exercises[0].name : 'Workout'));
